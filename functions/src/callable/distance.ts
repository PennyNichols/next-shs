...
import {
  COMPANY_ADDRESS,
  TRAVEL_DISTANCE_CHARGE,
  TRAVEL_DISTANCE_THRESHOLD,
  TRAVEL_DURATION_CHARGE,
  TRAVEL_DURATION_THRESHOLD,
} from '../constants';
...
export const getDrivingDistance = onCall({ region: 'us-east1', secrets: ['MAPS_API_KEY'] }, async (request) => {
  ...
  try {
    ...
    throw new HttpsError('internal', error.response?.data?.error_message || 'Could not calculate distance.');
  }
});
