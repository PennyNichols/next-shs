...
router.get('/', authenticate, async (req: Request, res: Response) => {
    try {
        ...
        if (!isAdminOrSuper(userRole)) {
            return res.status(403).json({ success: false, error: 'Forbidden: Insufficient permissions to view subscribers.' });
        }
...
router.delete('/:id', authenticate, async (req: Request, res: Response) => {
    try {
        ...
        // Check for necessary permissions
        if (!isAdminOrSuper(userRole)) {
            return res.status(403).json({ success: false, error: 'Forbidden: Insufficient permissions to delete subscribers.' });
        }
